...
contract BankContract is AccessControl, IERC721Receiver {
	...
	event BuyProp(address indexed to, uint256 indexed prop_id);
	event BuyBuild(address indexed to, uint256 indexed build_id, uint32 nb);
	event BuyPawn(address indexed to, uint256 indexed pawn_id);
	event SellProp(address indexed seller, uint256 indexed prop_id, uint256 price);
	event SellBuild(address indexed seller, uint256 indexed prop_id, uint256 price);
	event Withdraw(address indexed to, uint256 value);
	event EnrollPlayer(uint16 _edition, address indexed player);
	event MovePawn(address player, uint8 dices);
	...
	function buyPawn() external {
		...
		emit BuyPawn(msg.sender, pawn_id);
	}
	...
	function enrollPlayer(uint16 _edition) external {
		...
		emit EnrollPlayer(_edition, msg.sender);
	}
	...
	function rollDices(uint16 _edition) external {
		...
		emit MovePawn(msg.sender, dices);
	}
	...
	function buyProp(
		...
		emit BuyProp(msg.sender, prop_id);
	}
	...
	function sellProp(uint256 _id) public {
		...
		PropContract.Property memory property = Prop.get(_id);

		/* Bank buys PROP at 30% of selling price */
		uint256 price = (propPrices[property.edition][property.land][property.rarity] * 30) / 100;
		...
		emit SellProp(msg.sender, _id, price);
	}
	...
	function buyBuild(
		...
		emit BuyBuild(msg.sender, build_id, _amount);
	}

	function sellBuild(uint256 _id, uint32 _amount) public {
		BuildContract.Building memory building = Build.get(_id);

		/* Bank buys BUILD at 30% of selling price */
		uint256 price = (buildPrices[building.edition][building.land][building.buildType] * 30 * _amount) / 100;
		...
		emit SellBuild(msg.sender, _id, price);
	}
